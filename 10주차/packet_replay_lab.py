...
def capture_packets():
    ...
    if not iface:
        ...
        # 캡처 시작 직후 클라이언트 트래픽 발사
        if AUTO_FIRE_CLIENT:
            time.sleep(0.5)
            fire_one_client()

        sniff(iface=iface, prn=packet_callback, filter=f"tcp and host {HOST} and port {PORT}", timeout=SNIFF_TIMEOUT, store=False)
        print(f"[*] 캡처 완료. 총 {len(captured)} 개 패킷")

def replay_captured():
    if not captured:
        print("[!] 재연할 캡처된 패킷이 없습니다.")
        return
